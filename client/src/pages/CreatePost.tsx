...
const CreatePost: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const slugified = formData.title.split(' ')
                      ...
                      .join('-');
    try{
      const res = await axios.post(`${hostConfig.API_URL}/api/posts`, Object.assign(
  ...
  return (
    <>
    { loading ? <Loading /> : (
        <form onSubmit={handleSubmit} action="" className="max-w-xl mx-auto mt-36 flex flex-col min-h-44 gap-2 p-4 space-y-2">
          <h2 className="text-gray-900 text-xl" > Create Post </h2>
          {imagePreview && (
              <img src={imagePreview} className='w-48 h-32 object-cover rounded-md' alt="" />
          )}
          { errors.__html && (
